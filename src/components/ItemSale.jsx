...
const getData = async() => {
  try {
      let res = await fetch(`${NEXT_PUBLIC_HOST_URL}/api/landingPage/itemSale`);
      res = await res.json();
      return res;
  } catch(err) {
      console.error(err);
  ...
  return (
    <div className='h-[80vh] md:flex text-white gap-1 tracking-wider'>
       {data.length > 0 ? <div className='relative bg-cover bg-center h-3/4 md:h-full w-full md:w-1/2 flex flex-col gap-5 justify-end items-center pb-28'>
            <Image src={data[0].image} alt={data[0].title} fill style={{objectFit: 'cover'}}/>
            ...
            <div className='flex justify-center'><Link href='/collections/mediterranean_love' className='absolute top-[70%] border border-white bg-transparent hover:bg-white hover:text-black px-5 py-2 md:px-8 md:py-3'>Shop Our Collection</Link></div>
        ...
        <Link href='/featured/sale' className='border border-black bg-transparent hover:bg-black hover:text-white px-6 py-2 md:px-8 md:py-3 transition ease-in delay-50'>SHOP SALE</Link>
        </div>
    </div>
  )
}
