    ...
    <div className="bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800 sm:h-[calc(100vh-6rem)] sm:sticky sm:top-0 lg:min-w-[370px] sm:min-w-[300px] overflow-y-auto scrollbar-hide">
      <div className="flex flex-col h-full relative">
        ...
        <div className="bg-gray-800 w-full p-4">
          <h4 className="flex flex-wrap gap-4 text-base text-white">Total <span className="ml-auto">₹{data.reduce((total, payment) => total + payment.amount, 0)}</span></h4>
        </div>
      </div>
    </div>
      ...
      <form className="mt-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <input type="text" placeholder="First Name" name='firstName' value={formData.firstName} onChange={handleChange}
                className="px-4 py-3 bg-gray-100 focus:bg-transparent text-gray-800 w-full text-sm rounded-md focus:outline-black" />
            </div>
            <div>
        ...
        <div className="mt-8">
          ...
          <div className="flex gap-4 max-md:flex-col mt-8">
            ...
            <button type="button" className="rounded-md px-6 py-3 w-full text-sm tracking-wide border border-black bg-black hover:bg-white text-white hover:text-black ">Complete Purchase</button>
          </div>
        </div>
      </form>
    </div>
